...
import { XMarkCircleIcon, CheckCircleIcon} from "@heroicons/react/24/outline"
...
export default function ListaCirculaciones ({circulaciones, select, hover, onSelect, onHover}) {
  ...
  return(
    <div className='w-full mb-2 px-2 sm:px-4'>
      ...
      <table className="min-w-full">
        ...
        <tr key = {circulacion.id}
          ...
          <td className="hidden py-4 pl-1 sm:pl-3 text-sm sm:table-cell">
            {(circulacion.abierta)
              ? (<XMarkCircleIcon className="w-6 h-6 mx-auto my-1 text-red-600"/>)
              : (<CheckCircleIcon className="w-6 h-6 mx-auto my-1 text-emerald-500"/>)
            }
          </td>
          <td className="py-4 pl-2 pr-2 text-sm sm:pr-0">
            {(circulacion.alarma)
              ? (<BellAlertIcon className="w-6 h-6 mx-auto my-1 text-red-600"/>)
